...
static const char *TAG = "air_power";
...
{
    bool enabled = false;
    return aquapilot_settings_get_air_power_monitor_enabled(&enabled) && enabled;
}
...
static void refresh_status(void)
{
    ...
    if (shelly_client_get_plug_status(AQUAPILOT_SHELLY_AIR, &status) == ESP_OK) {
        s_plug_online = true;
...
static bool on_no_power_condition_met(void)
{
    ...
    if (!air_schedule_desired_plug_on()) {
        return false;
    }
    ...
    return s_cached_watts <= AIR_POWER_ALARM_WATTS_THRESHOLD;
}
...
static bool off_leak_condition_met(void)
{
    ...
    return s_cached_watts >= AIR_OFF_LEAK_WATTS_THRESHOLD;
}
...
{
    const esp_err_t err = shelly_client_plug_set(AQUAPILOT_SHELLY_AIR, false);
    if (err == ESP_OK) {
        ESP_LOGW(TAG, "air off-window leak: forced plug OFF");
        co2_automation_sync_now();
    } else {
        ESP_LOGW(TAG, "air off-window leak: force OFF failed (%s)", esp_err_to_name(err));
    }
}
...
{
    if (xTaskCreate(monitor_task, "air_power", 8192, NULL, 4, NULL) != pdPASS) {
        ESP_LOGE(TAG, "failed to create monitor task");
        return ESP_FAIL;
    }

    ESP_LOGI(TAG, "air power monitor started");
    return ESP_OK;
}
